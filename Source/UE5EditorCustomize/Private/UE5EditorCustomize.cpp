...
TArray<UObject*> FUE5EditorCustomizeModule::FindReferencedAsset()
{
    ...
    do
    {
        if(tmpRefs[0]->GetPathName().StartsWith("/UE5EditorCustomize/") && tmpRefs[0]->IsAsset() && !NeededObject.Contains(tmpRefs[0]))
        {
...
FString FUE5EditorCustomizeModule::GetAssetAbsolutePath(FString PackageName)
{
...
bool FUE5EditorCustomizeModule::_Internal_ImportUTheme_v0(TArray<uint8>& AllFileData, FText* ErrorMsg)
{
    //UTheme Head(_UTheme)
    //UTheme Version (0)
...
bool FUE5EditorCustomizeModule::_Internal_ExportUAsset(UObject* UAssetObj, TArray<uint8>& UThemeData)
{
...
bool FUE5EditorCustomizeModule::_Internal_ExportConfigIni(TArray<uint8>& UThemeData)
{
    TArray<uint8> FileData;
    FString ConfigIni = FPaths::ProjectSavedDir() + "/tmpUE5EditorCustomize.ini";
...
bool FUE5EditorCustomizeModule::_Internal_ExportTexture2D(UObject* TextureObj, TArray<uint8>& UThemeData)
{
    //Image Name
    //Image Target Path
    //Image Lens
    ...
    FString tmpBMPFileName = FPaths::ProjectSavedDir() + "/tmpUE5EditorCustomize.TGA";
...
bool FUE5EditorCustomizeModule::_Internal_ExportFont(UObject* FontObj, TArray<uint8>& UThemeData, TArray<UObject*>& NeededObject)
{
    //Default Font
...
bool FUE5EditorCustomizeModule::_Internal_ExportFontFace(const UObject* FontFaceObj, TArray<uint8>& UThemeData)
{
...
bool FUE5EditorCustomizeModule::_Internal_ImportUAsset(TArray<uint8>& UThemeData, int& Offset, FText* ErrorMsg)
{
...
bool FUE5EditorCustomizeModule::_Internal_ImportConfigIni(TArray<uint8>& UThemeData, int& Offset)
{
    FString ConfigIni = FPaths::ProjectSavedDir() + "/tmpUE5EditorCustomize" + FString::FromInt(rand()) + ".ini";
...
bool FUE5EditorCustomizeModule::_Internal_ImportTexture2D(TArray<uint8>& UThemeData, int& Offset)
{
    //Image Name
    //Image Target Path
    //Image Lens
...
bool FUE5EditorCustomizeModule::_Internal_ImportFont(TArray<uint8>& UThemeData, int& Offset)
{
...
UFontFace* FUE5EditorCustomizeModule::_Internal_ImportFontFace(TArray<uint8>& UThemeData, int& Offset)
{
    FString AssetPathName;
    AssetPathName = FString((TCHAR*)(UThemeData.GetData() + Offset));
    Offset += AssetPathName.Len() * 2 + 2;
...
bool FUE5EditorCustomizeModule::_Internal_LoadUThemeInfo(TArray<uint8>& UThemeData, int& Offset, int RequestVersion, int UThemeVersion, float Size)
{
...
bool FUE5EditorCustomizeModule::_Internal_AskUThemeInfo(FUThemeInfo_v0* OutInfo)
{
    if(!OutInfo)
        return false;
...
uint8 FUE5EditorCustomizeModule::ExtensionToUint8(FString FileExtension)
{
...
FString FUE5EditorCustomizeModule::Uint8ToExtension(uint8 Extension)
{
    switch(Extension)
    {
    case 0:
...
void FUE5EditorCustomizeModule::CacheOriginalBrushes()
{
...
void FUE5EditorCustomizeModule::ReCacheCustomBrushes()
{
...
void FUE5EditorCustomizeModule::StartupModule()
{
    ...
    FModuleManager::LoadModuleChecked<FPropertyEditorModule>("PropertyEditor").RegisterCustomClassLayout(UEditorCustomizeSetting::StaticClass()->GetFName(),
                                                                                                         FOnGetDetailCustomizationInstance::CreateLambda([]() {return MakeShareable(new UE5ECSettingsCustomization); }));
    SettingS = FModuleManager::LoadModuleChecked<ISettingsModule>("Settings").RegisterSettings("Project", "Plugins",
                                                                                               TEXT("UE5EditorCustomize"), FText::FromString("UE5 EditorCustomize"),
                                                                                               FText::FromString("Setting For UE5EditorCustomize"), GetMutableDefault<UEditorCustomizeSetting>());
    SettingS->OnModified().BindRaw(this, &FUE5EditorCustomizeModule::OnSettingModified);
    ...
    GetMutableDefault<UEditorCustomizeSetting>()->ReloadConfig(nullptr, nullptr, UE::LCPF_PropagateToInstances | UE::LCPF_PropagateToChildDefaultObjects);
    FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("ConfigReset_RestartEditor", "Config has been reset.Restart Editor to take effect."));
    return true;
    });
}

void FUE5EditorCustomizeModule::ShutdownModule()
{
    ...
    RestoreCachedBrush();
    FModuleManager::LoadModuleChecked<ISettingsModule>("Settings").UnregisterSettings("Project", "Plugins", "UE5EditorCustomize");
}

bool FUE5EditorCustomizeModule::OnSettingModified()
{
    UEditorCustomizeSetting* StyleSettings = GetMutableDefault<UEditorCustomizeSetting>();
    GetMutableDefault<UEditorStyleSettings>()->bUseGrid = StyleSettings->EditorUseGrid;
...
void FUE5EditorCustomizeModule::ApplyEditorStyle(UEditorCustomizeSetting* StyleSettings)
{
    FSlateStyleSet* EditorStyles = (FSlateStyleSet*)&FEditorStyle::Get();
...
void FUE5EditorCustomizeModule::ApplyCoreStyle(class UEditorCustomizeSetting* StyleSettings)
{
...
void FUE5EditorCustomizeModule::ApplyTextStyle(class UEditorCustomizeSetting* StyleSettings)
{
...
void FUE5EditorCustomizeModule::ApplyCustomStyle(class UEditorCustomizeSetting* StyleSettings)
{
    ReCacheCustomBrushes();
    //Apply Font
    FSlateStyleSet* EditorStyles = (FSlateStyleSet*)&FEditorStyle::Get();
    FSlateStyleSet* CoreStyles = (FSlateStyleSet*)&FCoreStyle::Get();
    auto tmpFunc = [&StyleSettings](FSlateStyleSet* SlateStyleSet, FUE5ECCustomStyle& CustomStyle)
    {
        //Apply Font First
...
bool FUE5EditorCustomizeModule::ImportSettingFromIni(FString FilePath)
{
    return (SettingS->Import(FilePath) && SettingS->Save());
}

bool FUE5EditorCustomizeModule::ImportUTheme(FString FilePath, FText* ErrorMsg)
{
    TArray<uint8> FileData;
    if(ErrorMsg)
...
void FUE5EditorCustomizeModule::ShowDialogForImport()
{
    TArray<FString> OutFiles;
    if(FDesktopPlatformModule::Get()->OpenFileDialog(NULL, "Import Theme",
       IPluginManager::Get().FindPlugin(TEXT("UE5EditorCustomize"))->GetBaseDir() + FString("/Themes")
...
void FUE5EditorCustomizeModule::ImportBuiltInTheme()
{
    if(ImportUTheme(IPluginManager::Get().FindPlugin(TEXT("UE5EditorCustomize"))->GetBaseDir() + FString("/Resources/HatsuneMiku.UTheme")))
    {
        FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("ImportSucceed", "Import Succeed."));
    }
    else
...
bool FUE5EditorCustomizeModule::PackageTheme(FString FilePath, FUThemeInfo_v0 UThemeInfo)
{
    //UTheme Head(_UTheme)
    //UTheme Version (0)
    //Requested Engine Version(e.g 19) (uint8)
...
void FUE5EditorCustomizeModule::ShowDialogForPackageTheme()
{
    FUThemeInfo_v0 UThemeInfo;
...
void FUE5EditorCustomizeModule::ResetEditorStyle()
{
...
void FUE5EditorCustomizeModule::ResetCoreStyle()
{
    FString ConfigName = GetMutableDefault<UEditorCustomizeSetting>()->GetClass()->GetConfigName();
    FString Sec = GetMutableDefault<UEditorCustomizeSetting>()->GetClass()->GetPathName();
...
void FUE5EditorCustomizeModule::ResetTextStyle()
{
    FString ConfigName = GetMutableDefault<UEditorCustomizeSetting>()->GetClass()->GetConfigName();
    FString Sec = GetMutableDefault<UEditorCustomizeSetting>()->GetClass()->GetPathName();
...
void FUE5EditorCustomizeModule::RestoreCachedBrush()
{
    FSlateStyleSet* EditorStyles = (FSlateStyleSet*)&FEditorStyle::Get();
    FSlateStyleSet* CoreStyles = (FSlateStyleSet*)&FCoreStyle::Get();
...
IMPLEMENT_MODULE(FUE5EditorCustomizeModule, UE5EditorCustomize)